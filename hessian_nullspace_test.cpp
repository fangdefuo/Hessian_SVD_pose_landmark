...
int main()
{
    ...
    std::vector<Pose> camera_pose;//6维
    double radius = 8;
    for(int n = 0; n < poseNums; ++n ) {
        ...
        camera_pose.push_back(Pose(R,t));//定义相机的旋转和平移
    }
    ...
    for(int j = 0; j < featureNums; ++j)
    {
        ...
        Eigen::Vector3d Pw(tx, ty, tz);//定义的路标点
        points.push_back(Pw);

        for (int i = 0; i < poseNums; ++i) {
            Eigen::Matrix3d Rcw = camera_pose[i].Rwc.transpose();
            Eigen::Vector3d Pc = Rcw * (Pw - camera_pose[i].twc);//旋转之后的点坐标　＝＝　（　之前的点坐标　－　相机的平移　）* 相机旋转矩阵
